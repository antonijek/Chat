...
<div class="container justify-center bg-success-subtle p-5 mt-5 align-items-center d-flex rounded" style="height: 300px; width: 500px">
    <form method="post" action="{{route('connection.store')}}">
        @csrf
        <div>
            <input type="hidden" name="me" class="form-control" aria-describedby="emailHelp" value="{{Auth::user()->id}}">
        </div>

        <div >
            <input type="hidden" name="friend" class="form-control" value="{{$user->id}}">
        </div>

        <div class="text-center align-self-center">
            <h4>Da li zelite da posaljete zahtjev za prijatejstvo korisniku {{$user->firstName}} {{ $user->lastName}}</h4>
