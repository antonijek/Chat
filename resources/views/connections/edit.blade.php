...
<x-app-layout>
    ...
    <div class="container justify-center bg-success-subtle p-5 mt-5 align-items-center d-flex rounded" style="height: 300px; width: 500px">
        <form method="POST" action="{{ route('connection.update',$connection->id) }}">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <input type="hidden" name="status" value="accepted" class="form-control" aria-describedby="emailHelp">
            </div>
            <div class="mb-3">
            ...
            <h4>Da li zelite da prihvatie zahtjev za prijatejstvo korisnika {{User::find($connection->sender_id)->firstName}} {{ User::find($connection->sender_id)->lastName}}</h4>
